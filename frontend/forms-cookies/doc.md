...
**`Хранение кук`**
В современных браузерах cookies сохраняются в базах данных (SQLite, Chrome, Firefox). Это не отдельные текстовые файлы, а записи в таблице внутри файла вроде Cookies (в папке браузера).
...
3.3 Моя версия deleteAllCookies
```javascript
function deleteAllCookies() {
    let currentName;
    ...
    for (cookie of cookiesArray) {
        currentName = cookie.trim();
        document.cookie = `${currentName.substring(0, currentName.indexOf("="))}= ; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;`;
    }
}
...
function getSecureCookie(name) {
  ...
  for (let cookie of cookiesArray) {
    if (cookie.trim().indexOf(nameEQ) === 0) return decodeURIComponent(cookie.substring(nameEQ.length));
  }
  return null;
}

console.log(getSecureCookie("username"));
...
document.cookie =
  "history=" + encodeURIComponent(JSON.stringify(largeData)) + "; path=/";
...
В JS оба цикла используются для итерации, но работают по-разному и предназначены для разных типов данных. `for...in` появлися раньше, а `for...of` - в ES6 (2015) для более удобной работы с итерируемыми объектами.

**`for...in`**
Перебирает ключи (свойства объекта). Для массивов - индексы (как строки, например, "0", "1"). Работает с любым объектом (включая массивы, но с нюансами).
Особенности:
- Перебирает все перечисляемые свойства, включая унаследованные (от прототипа).
- Для массивов может быть неэффективным, так как индексы - строки, и порядок не гарантирован (хотя в практике для массивов он обычно сохраняется).
- Не рекомендуется для массивов, если нужен порядок или только собственные свойства.
```javascript
const arr = [10, 20, 30];
...
**`for...of`**
Перебирает значения элементов итерируемого объекта. Работает с итерируемыми объектами (массивы, строки, Map, Set, генераторы, TypedArray и т.д.). Не работает с обычными объектами (выдаст ошибку `TypeError: obj is not iterable`). 

Особенности: перебирает только значения, в порядке итерации (для массивов - по индексам). Не затрагивает ключи или унаследованные свойства, более эффективен и читаем для коллекций.

```javascript
const arr = [10, 20, 30]
for (let value of arr) {
    console.log('value :>> ', value); // 10, 20, 30 (значения)
}
...
***Использование***
`for...in`:
- Для перебора ключей объекта (проверка или удаление свойств);
- Когда нужно работать с унаследованными свойствами;

`for...of`:
- Для массивов, строк, Map, Set - когда нужны значения.

**Не использовать for...in для массивов! Лучше for..of или forEach, чтобы избежать проблем с индексами и порядком**.
for...in - для ключей объекта (старый, но гибкий)
for...of - для значений итерируемых коллекций (новый, более безопасный и удобный)