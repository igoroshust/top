...
- `grid-auto-flow` - выстраивание "направления" контента. По умолчанию row, можно указать column.
...
Неявная сетка - разметка, которая распределяется внутри блока по "остаточному принципу" - к ней не применяются указанные стили.
