...
```
Комментарии
- `oninput` - событийный атрибут, срабатывающий каждый раз, когда пользователь меняет содержимое поля ввода (набирает, удаляет, вставляет текст)
