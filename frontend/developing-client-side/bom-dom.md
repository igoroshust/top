...
- **Объекты**: Основной - `window` (глобальный объект, представляющий окно браузера). Другие: `navigator` (информация о браузере), `location` (URL и навигация), `history` (история просмотров), `screen` (экран устройства).
...
- Ограничения. В строгих режимах (напримре, CSP) некоторые BOM-функции могут быть заблокированы. DOM может быть медленным при больших деревьях – используйте виртуальный DOM (как в React).
